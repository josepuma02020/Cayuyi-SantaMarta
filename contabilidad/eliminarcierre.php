...
if ($_SESSION['usuario']) {
    ...
    $id = $_POST['id'];
    $consultaefecivo_ = "select base,ruta from revisionesrutas where idhistorial=$id";
    $queryefectivo = mysqli_query($link, $consultaefecivo_) or die($consultaefecivo_);
    $filaefectivo = mysqli_fetch_array($queryefectivo);

    $consultadescuentobase = "UPDATE `rutas` SET `base`=$filaefectivo[base]  WHERE `id_ruta` = $filaefectivo[ruta]";
